	...
	return (
		<div className="overflow-x-auto rounded-xl border border-border">
			<table className="w-full border-collapse text-sm">
					...
					{criteria.map((criterion) => {
						...
						return (
							...
							>
								...
								{houses.map((house) => {
									const score =
										criterion.scoreByHouseId[house.id] ??
										null;
									const isBest =
										score !== null &&
										maxScore !== null &&
				...
				<tfoot>
					<tr className="border-t-2 border-border bg-[color-mix(in_srgb,var(--muted)_50%,var(--background))] font-semibold">
						<td
							className="sticky left-0 z-10 bg-[color-mix(in_srgb,var(--muted)_50%,var(--background))] px-4 py-3 font-semibold text-foreground"
							colSpan={1}
						>
						...
						{houses.map((house) => {
							...
							const isBest =
								...
								maxFinal !== null &&
								score === maxFinal &&
								allFinalScores.filter((s) => s === maxFinal)
									.length === 1;
